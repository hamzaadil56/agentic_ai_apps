...
router_model = ChatGoogleGenerativeAI(
    model="gemini-1.5-flash-8b-001", api_key=api_key)
...
def classify_question(question: str) -> QuestionType:
    ...
    response = cast(QuestionClassifier, router_model.with_structured_output(
        QuestionClassifier).invoke(prompt.format(question=question)))
...
def main():
    result = run_workflow.invoke(input={"question": "What is 2+2?"})
